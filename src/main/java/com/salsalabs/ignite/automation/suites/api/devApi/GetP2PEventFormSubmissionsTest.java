...
import com.salsalabs.ignite.automation.apiautomation.models.devApiModels.submissions.p2peventform.Request.P2PEventFormSubmissionsRequest;
import com.salsalabs.ignite.automation.apiautomation.models.devApiModels.submissions.p2peventform.Response.P2PEventFormsSubmissions;
...
public class GetP2PEventFormSubmissionsTest extends CommonTest {
    ...
    public void getTestData() throws IOException {
        loadExpectedResultObj(
                new HashMap<String, Class<? extends ExpectedResult>>() {{
                    put(TEST_DATA_PATH_SUBMISSIONS + "p2p_event_form_submissions_response.json", P2PEventFormsSubmissions.class);
    ...
    @Parameters({"ENV"})
    @Test(priority = 1)
    public void getP2PEventFormSubmissions(String env) throws IOException {

        P2PEventFormSubmissionsRequest requestObj = new ObjectMapper().readValue(new File(TEST_DATA_PATH_SUBMISSIONS + "p2p_event_form_submissions_request.json"), P2PEventFormSubmissionsRequest.class);
        ResponseEntity<P2PEventFormsSubmissions> response =
                restClient.exchange(env + Config.Endpoints.LIST_OF_SUBMISSIONS, HttpMethod.POST, buildRequest(requestObj), P2PEventFormsSubmissions.class);

        P2PEventFormsSubmissions expectedResultObj = (P2PEventFormsSubmissions) getExpectedResult("p2p_event_form_submissions_response.json");
