	 ...
	 @BeforeTest
	    public void getTestData() throws IOException {
	        loadExpectedResultObj(
	                ...
	                {{
	                    put(TEST_DATA_PATH_ATTENDEE_SUMMARY_RESPONSE + "get_event_attendee_summary.json", GetEventAttendeeResponse.class);
	  ...
	  @Parameters({"ENV"})
	   ...
	   @Parameters({"ENV" , "UUID"})
	    ...
	    public void getEventAttendeeSummary(String env, String uuid) throws IOException {
	        ResponseEntity<GetEventAttendeeResponse> response =
	             restClient.exchange(env + Endpoints.ACTIVITYCORESUMMARY+ uuid+"/summary/attendees", HttpMethod.GET, buildRequest(null), GetEventAttendeeResponse.class);
	        GetEventAttendeeResponse expectedResultObj = ((GetEventAttendeeResponse) getExpectedResult("get_event_attendee_summary.json"));
	   ...
	   @Parameters({"ENV"})
	    ...
	    public void getMetricsAfterExecution(String env) throws IOException {
	        ...
	        metricsBeforeExecution.getPayload().setEventAttendeesList(metricsBeforeExecution.getPayload().getEventAttendeesList()+1);
